#!/bin/bash
# Build flashinfer for RTX 4080/4090 (Ada Lovelace, SM89) only.
...
# binary -- see docs) with FLASHINFER_CUDA_ARCH_LIST pinned to sm_89, so
...
if [ "${COMPUTE_CAP}" != "8.9" ]; then
  echo "ERROR: detected GPU compute capability '${COMPUTE_CAP}', expected '8.9' (RTX 40 / Ada Lovelace)." >&2
  echo "This script targets the native-builds branch; use main for other GPUs." >&2
  exit 1
fi

# Ada Lovelace has no "a" (family-specific) SASS variant -- unlike Blackwell,
# there's nothing beyond the compatible/generic sm_89 target to opt into.
export FLASHINFER_CUDA_ARCH_LIST="8.9"

echo "=========================================="
echo "Building flashinfer for RTX 40 (SM89) only"
echo "=========================================="
...
echo "::group::Install flashinfer-python (editable, RTX 40-only)"
...
echo "::group::Build flashinfer-python wheel (+rtx40 local version) for external consumers, e.g. vllm"
...
FLASHINFER_LOCAL_VERSION=rtx40 python3 -m build --wheel --no-isolation "${REPO_ROOT}"
echo "Built wheel(s):"
...
echo "  <vllm-venv>/bin/pip install --force-reinstall ${REPO_ROOT}/dist/flashinfer_python-*+rtx40-*.whl"
